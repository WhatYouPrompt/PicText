...
- 每一行字幕都对应一张独立的“字幕条”，第一行位于原图底部，后续行截取原图底部图像并在下方依次拼接，形成“电影台词长图”风格。
...
### 2. 字幕设置
- 字幕高度（px）：单行字幕条的高度。第一行占用原图底部该高度区域，后续每行截取原图底部相同高度区域拼接。
- 字体大小（px）：影响每行文字排版；不自动换字号，超出时给出溢出提示。
- 字体颜色：统一控制所有行文字的填充色；颜色预览直接显示在选项框背景中。
- 背景条颜色（弹窗）：统一控制所有行背景条的颜色；点击选项框弹出面板，面板内包含颜色选择与透明度滑块。默认颜色为黑色 (#000000)，透明度默认为 20%。选项框本体使用棋盘底纹+颜色叠加展示当前透明度效果。
...
- 重置按钮：点击标题栏右侧的“重置”图标，清空输入框内容并重置预览。
...
### 1. 电影感拼接效果
- 自上而下拼接生成长图：
  - **第一行字幕**：直接覆盖在原图底部的背景条区域（高度 = 字幕高度）。
  - **后续行字幕**：每一行都截取原图最底部（高度 = 字幕高度）的图像区域，作为新画布拼接在上方图像的底部。
  - **总高度变化**：生成的长图总高度 = 原图高度 + (行数 - 1) × 字幕高度。
- 视觉效果：
  - 每一行不仅包含文字和背景条，还包含底部的画面，形成经典的“电影截图拼接”效果。

### 2. 文本排版
- 水平对齐：居中（可扩展左/右对齐）。
- 垂直对齐：文字在对应背景条垂直居中；条内留 `paddingTop/Bottom = max(4px, round(fontSize*0.2))`。
- 溢出处理：若 `fontSize + 内边距 > 字幕高度`，显示红色提示“字体过大，请增大字幕高度或减小字号”。
...
### 5. 导出一致性
- **宽高一致性**：预览区域的画布比例必须与当前显示的内容（原图或生成图）的实际像素比例严格一致，禁止拉伸或挤压。

## 八、参数范围与默认值
- 字幕高度：默认 100px（表示每一行的高度）。
...
## 十三、验收标准
- 预览与导出完全一致：生成后的预览图应当展示完整的长图效果，且比例正常（无纵向压缩）。
