...
const bgPopoverDone=document.getElementById('bgPopoverDone');
...
let img=null;let imgName='image';let offscreen=null;let scale=1;let pending=false;let hasGenerated=false;let backgroundAlpha=1;
...
function hexToRgb(hex){const h=hex.replace('#','');const r=parseInt(h.substring(0,2),16);const g=parseInt(h.substring(2,4),16);const b=parseInt(h.substring(4,6),16);return {r,g,b}}
function updateColorFields(){
  fontColorField.style.backgroundColor=fontColorInput.value;
  const {r,g,b}=hexToRgb(bgColorPicker.value);
  bgOverlay.style.backgroundColor=`rgba(${r},${g},${b},${backgroundAlpha})`;
...
function render(){
 ...
 const captionHeight=Number(captionHeightInput.value)||0;const fontSize=Number(fontSizeInput.value)||12;const fontColor=fontColorInput.value;const bgColor=bgColorPicker.value;const fontFamily=fontFamilySelect.value;const fontWeight=fontWeightSelect.value;const lines=readLines();
...
bgColorField.addEventListener('click',()=>{bgPopover.classList.remove('hidden')});
bgPopoverDone.addEventListener('click',()=>{bgPopover.classList.add('hidden');onParamsChange();updateColorFields()});
bgColorPicker.addEventListener('input',()=>{updateColorFields()});
bgAlphaRange.addEventListener('input',()=>{backgroundAlpha=clamp(Number(bgAlphaRange.value)/100,0,1);updateColorFields()});
...
generateBtn.addEventListener('click',()=>{if(!img){showToast('请先上传图片',false);return}const lines=readLines();const nonEmpty=lines.filter(l=>l.length>0);if(nonEmpty.length===0){showToast('请输入字幕内容',false);return}const captionHeight=Number(captionHeightInput.value)||0;const fontSize=Number(fontSizeInput.value)||12;const fontColor=fontColorInput.value;const bgColor=bgColorPicker.value;const fontFamily=fontFamilySelect.value;const fontWeight=fontWeightSelect.value;const w=img.width,h=img.height;const bars=lines.length>0?lines.length:captionHeight>0?1:0;const barH=bars>0?computeBarHeight(captionHeight,lines):0;const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d');ctx.imageSmoothingEnabled=true;ctx.drawImage(img,0,0,w,h);if(bars>0&&barH>0){const {r,g,b}=hexToRgb(bgColor);ctx.textAlign='center';ctx.textBaseline='middle';ctx.font=`${fontWeight} ${fontSize}px ${fontFamily}`;ctx.lineJoin='round';ctx.miterLimit=2;for(let i=0;i<bars;i++){const y=h-captionHeight+i*barH;ctx.fillStyle=`rgba(${r},${g},${b},${backgroundAlpha})`;ctx.fillRect(0,y,w,barH);const ty=y+barH/2;const text=lines[i]||'';if(text){ctx.fillStyle=fontColor;ctx.fillText(text,w/2,ty)}}}offscreen=canvas;hasGenerated=true;showToast('字幕图片生成成功!');render()});
